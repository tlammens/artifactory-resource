...
func (c Check) RetrieveVersions(results []commands.SearchResult) ([]chelper.Version, error) {
	...
	if c.source.Version == "" {
		for _, file := range results {
			versions = append(versions, utils.PathToVersion(file.Path))
		}
		return versions, nil
	}
	semverPrevious := c.RetrieveSemverFilePrevious()
	if semverPrevious.Path != "" {
		versions = append(versions, utils.PathToVersion(semverPrevious.Path))
	}
...
func (c Check) SemverFilesToVersions(semverFiles []SemverFile) []chelper.Version {
	...
	for _, fileSemver := range semverFiles {
		versions = append(versions, utils.PathToVersion(fileSemver.Path))
	}
	return versions
}
func (c Check) RetrieveSemverFilePrevious() SemverFile {
	semverFile, _ := c.SemverFromPath(utils.VersionToPath(c.cmd.Version()))
	return semverFile
}
